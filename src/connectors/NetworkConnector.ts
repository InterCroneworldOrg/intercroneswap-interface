...
class MiniRpcProvider implements AsyncSendable {
    ...
    batch = batch.filter((b) => {
      if (b.request.method === 'wallet_switchEthereumChain') {
        try {
          ...
          invariant(err instanceof Error, 'Not an error got caught');
          b.reject(err);
        }
        return false;
      }
  ...
  public readonly sendAsync = (
    ...
    this.request(request.method, request.params)
      .then((result: any) => callback(null, { jsonrpc: '2.0', id: request.id, result }))
...
export class NetworkConnector extends AbstractConnector {
  ...
  constructor({ urls, defaultChainId }: NetworkConnectorArguments) {
    invariant(defaultChainId || Object.keys(urls).length === 1, 'defaultChainId is required argument with >1 url');
