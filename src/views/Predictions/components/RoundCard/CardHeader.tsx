...
export const getBorderBackground = (theme: DefaultTheme, status: Status) => {
  ...
  switch (status) {
    case 'calculating':
      return `linear-gradient(transparent ${gradientStopPoint}, ${theme.colors.cardBorder} ${gradientStopPoint}), ${theme.colors.gradients.grey}`
    case 'canceled':
...
const getBackgroundColor = (theme: DefaultTheme, status: Status) => {
  switch (status) {
    case 'calculating':
      return theme.colors.gradients.grey
    case 'live':
      return 'transparent'
    case 'canceled':
