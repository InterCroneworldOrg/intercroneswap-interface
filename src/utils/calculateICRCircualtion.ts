...
import { TokenAmount, Token } from '@intercroneswap/v2-sdk';
...
export function computeIcrCirculation(
  icr: Token,
  blockTimestamp: BigNumber,
  unclaimedIcr: TokenAmount | undefined,
): TokenAmount {
  ...
  const total = new TokenAmount(icr, JSBI.multiply(wholeAmount, JSBI.exponentiate(JSBI.BigInt(10), JSBI.BigInt(18))));
  return unclaimedIcr ? total.subtract(unclaimedIcr) : total;
}
