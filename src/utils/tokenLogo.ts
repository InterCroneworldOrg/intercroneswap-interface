...
export const getTokenLogoURL = (address: string, allTokens: TokenList[]): string[] => {
  if (address.toLocaleLowerCase() === PLZ.address.toLowerCase())
    return ['https://static.tronscan.org/production/upload/logo/new/TYK71t3eD1pTxpkDp7gbqXM5DYfaVdfKjV.png'];
  ...
  const coin_top_main_url = `https://static.tronscan.org/production/upload/logo/${ethAddress.toTron(address)}.`;
  const static_tronscan_url = `https://static.tronscan.org/production/upload/logo/${ethAddress.toTron(address)}.`;
