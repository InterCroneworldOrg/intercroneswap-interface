...
import { SwapParameters, Trade } from '@intercroneswap/v2-sdk'
      ...
      callback: async function onSwap(): Promise<string> {
        const estimatedCalls: SwapCallEstimate[] = await Promise.all(
          swapCalls.map((call) => {
            ...
            const firstArg = BigNumber.from(args[0]);
            if( firstArg.lt( BigNumber.from(2000)) )
              args[0] = firstArg.add(BigNumber.from(1)).toHexString();
